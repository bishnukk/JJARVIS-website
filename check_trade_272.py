...
print("Checking orders for jr_trade_id: 272...")
url = f"{SUPABASE_URL}/rest/v1/orders?jr_trade_id=eq.272"
response = requests.get(url, headers=headers)
if response.status_code == 200:
    data = response.json()
    for item in data:
        print(f"ID: {item.get('id')}, Date: {item.get('order_date')}, Side: {item.get('jr_order_side')}, Status: {item.get('status')}, Price: {item.get('jr_price')}")
else:
