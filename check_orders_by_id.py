...
print("Checking Orders by ID (Latest 20)...")
url = f"{SUPABASE_URL}/rest/v1/orders?select=*&order=id.desc&limit=20"
response = requests.get(url, headers=headers)
if response.status_code == 200:
    data = response.json()
    for item in data:
        print(f"ID: {item.get('id')}, Date: {item.get('order_date')}, Created: {item.get('created_at')}, Symbol: {item.get('symbol')}, Status: {item.get('status')}")
else:
